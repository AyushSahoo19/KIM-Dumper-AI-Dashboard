...
window.VIEWS = {
  ...
  _renderScatterPlot(rows, mode){
    ...
    const colorbar = document.getElementById('gm-scatter-colorbar');
    const stats = document.getElementById('gm-scatter-stats');
    const tag = document.getElementById('gm-heat-mode-tag');
    const tip = document.getElementById('gm-scatter-tooltip');
    if(!canvas) return;
  ...
  _renderGmapFuelSpeedHeatmap(rows, mode){
    ...
    this._renderScatterPlot(rows, mode);
  },
};
